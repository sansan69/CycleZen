...
/** Minimal saved route data for AI analysis */
export interface SavedRouteForAnalysis {
  distanceKm: number;
...
/**
 * Analyze a user's saved routes and generate personalized recommendations.
 * Uses Genkit + Gemini to understand riding patterns and suggest new routes.
 */
export async function getAIRecommendations(
  savedRoutes: SavedRouteForAnalysis[]
): Promise<RouteRecommendation[]> {
  // Don't call AI if user has no saved routes
  if (!savedRoutes || savedRoutes.length === 0) return [];

  // Build a summary of the user's riding profile
  const avgDistance =
...
/** Simple mode function — returns the most frequent item in an array */
